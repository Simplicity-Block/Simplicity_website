    ...
    <section className="py-32 relative">
      <div className="max-w-7xl mx-auto px-8">
        ...
        <div className="relative">
          {/* Timeline line */}
          <div className="absolute left-[calc(50%-1px)] top-0 bottom-0 w-0.5 bg-gradient-to-b from-purple-500 to-blue-500 opacity-20" />
          
          <div className="space-y-24">
            {timelineItems.map((item, index) => (
              <div key={index} className="relative grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className={`${index % 2 === 0 ? 'md:text-right' : ''} ${index % 2 === 0 ? 'md:order-1' : 'md:order-2'}`}>
                  <div className={`inline-flex items-center space-x-4 ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                ...
                <div className={`hidden md:flex justify-center items-center ${index % 2 === 0 ? 'order-2' : 'order-1'}`}>
